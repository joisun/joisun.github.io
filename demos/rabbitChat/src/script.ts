...
class TalkWindow {
  ...
  isTyping: boolean
  messages: Ele
  uctTimer: number | null
  ws: WebSocket
  me: { nickname: string; id: string }
  constructor(
    ws: WebSocket,
    me: { nickname: string; id: string }
  ) {
...
class Dialog {
  instance: HTMLDialogElement
  ws: WebSocket
  constructor(cb: (nickName: string) => void) {
    this.instance = $('#favDialog').get(
      0
...
function init() {
  ...
  function okDialog(nickName: string) {
    const ws = new WebSocket(
      'wss://jaycethanks-github-io-8zio.vercel.app'
    )
