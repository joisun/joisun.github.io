...
const readDir = async (dir:FileSystemDirectoryHandle,deep=1)=>{
    const Item:ItemType = {
        name:dir.name,
        folderList:[],
        fileList:[],
...
const generateTreeString = (tree:ItemType,intend="",isLast=false)=>{
...
// 生成树形字符串的函数
const generateTreeString2 = (node:ItemType, indent = '', isLast = true) =>{
    const marker = isLast ? '└── ' : '├── ';
